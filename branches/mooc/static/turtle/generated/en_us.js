...
turtlepage.showInterstitials = function(opt_data, opt_ignored, opt_ijData) {
  ...
  switch (opt_ijData.reinf[1]) {
    ...
    case 'picture-table':
      output += '<table>';
      var tupleList245 = opt_ijData.reinf[2];
      var tupleListLen245 = tupleList245.length;
      for (var tupleIndex245 = 0; tupleIndex245 < tupleListLen245; tupleIndex245++) {
        var tupleData245 = tupleList245[tupleIndex245];
        output += '<tr height="100" valign="middle"><td><img src="' + soy.$$escapeHtml(tupleData245[1]) + '"></td><td>' + soy.$$escapeHtml(tupleData245[0]) + '</td></tr>';
      }
      output += '</table>';
      break;
    case 'quiz':
      ...
      var iLimit261 = opt_ijData.reinf[2][0] + 1;
      for (var i261 = 1; i261 < iLimit261; i261++) {
        output += '<td><img src="p' + soy.$$escapeHtml(opt_ijData.page) + '-l' + soy.$$escapeHtml(opt_ijData.level) + '-a' + soy.$$escapeHtml(i261) + '.png" class="answer" onclick="alert(' + ((i261 == opt_ijData.reinf[2][1]) ? '\'You got it!\'); document.getElementById(\'continueButton\').style.display = \'inline\';' : '\'Wrong!  Try again.\');') + '"></td>';
      }
      output += '</tr></table>';
      break;
    case 'animations':
      ...
      var fileList287 = opt_ijData.reinf[2];
      var fileListLen287 = fileList287.length;
      for (var fileIndex287 = 0; fileIndex287 < fileListLen287; fileIndex287++) {
        var fileData287 = fileList287[fileIndex287];
        output += '<td><img src="' + soy.$$escapeHtml(fileData287) + '"></td>';
      }
      output += '</tr></table></div>';
      break;
    case 'special':
...
turtlepage.startBlocks = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_ijData.page == 1) {
    ...
    if (opt_ijData.level == 3 || opt_ijData.level == 5) {
      ...
      for (var i357 = 25; i357 < 61; i357 += 5) {
        output += '<block type="draw_move" ' + ((i357 == 25) ? 'x="300" y="100"' : '') + ' inline="false" editable="false" disabled="true"><title name="DIR">moveForward</title><value name="VALUE"><block type="math_number" disabled="true"><title name="NUM">' + soy.$$escapeHtml(i357) + '</title></block></value><next><block type="draw_turn" inline="false" editable="false" disabled="true"><title name="DIR">turnRight</title><value name="VALUE"><block type="math_number" disabled="true"><title name="NUM">90</title></block></value><next>';
      }
      for (var i365 = 5; i365 < 25; i365 += 5) {
...
turtlepage.start = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_ijData.reinf) {
    ...
    output += '</span> &nbsp; ';
    for (var i566 = 1; i566 < 11; i566++) {
      output += ' ' + ((i566 == opt_ijData.level) ? (i566 > 9) ? '<span class="selected doubleDigit tab">' + soy.$$escapeHtml(i566) + '</span>' : '<span class="selected tab">' + soy.$$escapeHtml(i566) + '</span>' : (i566 < opt_ijData.level) ? '<a class="tab previous" href="?lang=' + soy.$$escapeHtml(opt_ijData.lang) + '&page=' + soy.$$escapeHtml(opt_ijData.page) + '&level=' + soy.$$escapeHtml(i566) + '">' + soy.$$escapeHtml(i566) + '</a>' : '<a class="tab" href="?lang=' + soy.$$escapeHtml(opt_ijData.lang) + '&page=' + soy.$$escapeHtml(opt_ijData.page) + '&level=' + soy.$$escapeHtml(i566) + '">' + soy.$$escapeHtml(i566) + '</a>');
    }
