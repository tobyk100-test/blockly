...
turtlepage.start = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_ijData.page == 1) {
    ...
    if (opt_ijData.level == 3 || opt_ijData.level == 5) {
      ...
      for (var i177 = 25; i177 < 61; i177 += 5) {
        output += '<block type="draw_move" ' + ((i177 == 25) ? 'x="300" y="100"' : '') + ' inline="false" editable="false" disabled="true"><title name="DIR">moveForward</title><value name="VALUE"><block type="math_number"><title name="NUM">' + soy.$$escapeHtml(i177) + '</title></block></value><next><block type="draw_turn" inline="false" editable="false" disabled="true"><title name="DIR">turnRight</title><value name="VALUE"><block type="math_number"><title name="NUM">90</title></block></value><next>';
      }
      for (var i185 = 5; i185 < 25; i185 += 5) {
  ...
  output += '</div><table width="100%" height="100%"><tr><td width="' + ((opt_ijData.reinf && opt_ijData.reinf != 0) ? '100%' : '410') + '" valign="top"><h1 class="nav-menu"><a href="../">MOOC</a> &gt;  Turtle ' + soy.$$escapeHtml(opt_ijData.page) + ' </h1>';
  if (opt_ijData.reinf) {
    ...
    switch (opt_ijData.reinf[1]) {
      ...
      case 'picture-table':
        output += '<table>';
        var tupleList282 = opt_ijData.reinf[2];
        var tupleListLen282 = tupleList282.length;
        for (var tupleIndex282 = 0; tupleIndex282 < tupleListLen282; tupleIndex282++) {
          var tupleData282 = tupleList282[tupleIndex282];
          output += '<tr height="100" valign="middle"><td><img src="' + soy.$$escapeHtml(tupleData282[1]) + '"></td><td>' + soy.$$escapeHtml(tupleData282[0]) + '</td></tr>';
        }
        output += '</table>';
        break;
      case 'quiz':
        ...
        var iLimit298 = opt_ijData.reinf[2][0] + 1;
        for (var i298 = 1; i298 < iLimit298; i298++) {
          output += '<td><img src="p' + soy.$$escapeHtml(opt_ijData.page) + '-l' + soy.$$escapeHtml(opt_ijData.level) + '-a' + soy.$$escapeHtml(i298) + '.png" class="answer" onclick="alert(' + ((i298 == opt_ijData.reinf[2][1]) ? '\'You got it!\'); document.getElementById(\'continueButton\').style.display = \'inline\';' : '\'Wrong!  Try again.\');') + '"></td>';
        }
        output += '</tr></table>';
        break;
      case 'animations':
        ...
        var fileList324 = opt_ijData.reinf[2];
        var fileListLen324 = fileList324.length;
        for (var fileIndex324 = 0; fileIndex324 < fileListLen324; fileIndex324++) {
          var fileData324 = fileList324[fileIndex324];
          output += '<td><img src="' + soy.$$escapeHtml(fileData324) + '"></td>';
        }
        output += '</tr></table></div>';
        break;
      case 'special':
    ...
    output += '<p class="levelLine nav-menu">Level: &nbsp;';
    var iLimit346 = opt_ijData.maxLevel + 1;
    for (var i346 = 1; i346 < iLimit346; i346++) {
      output += (i346 == opt_ijData.level) ? '<span class="tab" id="selected">' + soy.$$escapeHtml(i346) + '</span>' : '<a class="tab" href="?page=' + soy.$$escapeHtml(opt_ijData.page) + '&level=' + soy.$$escapeHtml(i346) + '">' + soy.$$escapeHtml(i346) + '</a>';
    }
